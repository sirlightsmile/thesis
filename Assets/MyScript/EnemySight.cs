...
public class EnemySight : MonoBehaviour {
	...
	void Awake(){
		...
		col2 = gameObject.GetComponent<CapsuleCollider> ();
	...
	void OnTriggerEnter (Collider other){
		if (other.tag == "Player" && gameObject.GetComponent<EnemySight> ().playerInSight == true) {
	...
	void OnTriggerStay (Collider other)
	{
		...
		if (other.gameObject == player) {
			...
			if (angle < fieldOfViewAngle * 0.5f) {
				...
				if (Physics.Raycast (StartDirection, direction.normalized, out hit, col.radius)) {
